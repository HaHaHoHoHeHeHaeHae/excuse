					...
					<div style="height: 300px;">
						<table class="alt"
							...
							<tbody>
								...
								<tr>
									<td><b>대표자명</b></td>
									<td><a style="text-decoration:none; cursor:pointer;" onclick="OpenWindow('memberdetail?mem_id=${member.mem_id}','','430','680')">${member.mem_nick }</a></td>
								</tr>
								<tr>
									<td><b>상태</b></td>
									<td><c:if test="${club.club_status==0}">승인대기</c:if> <c:if
											test="${club.club_status==1 }">운영중</c:if> <c:if
											test="${club.club_status==2 }">운영중지</c:if></td>
								</tr>
							</tbody>
						</table>
					</div>
